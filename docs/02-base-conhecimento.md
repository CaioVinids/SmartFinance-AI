...
Antes de serem utilizados pelo agente, os dados passam por um pré-processamento para gerar informações mais relevantes e reduzir complexidade.

Esse processo inclui:

- Agregação de gastos por categoria
- Cálculo de indicadores financeiros (percentual de gastos, capacidade de poupança)
- Identificação de despesas fixas e variáveis
- Análise de padrões de consumo

Essas informações são utilizadas para enriquecer o contexto enviado ao modelo, permitindo respostas mais assertivas e personalizadas.
...
O agente utiliza uma abordagem híbrida de contexto:

Contexto estático: informações essenciais do usuário (perfil, metas e resumo financeiro) são injetadas no prompt para guiar o raciocínio do modelo.

Contexto dinâmico: dados detalhados (transações, produtos e histórico) são consultados sob demanda, garantindo respostas mais precisas sem sobrecarregar o limite de contexto do modelo.
